  ...
  const handleGoogleLogin = async () => {
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: { redirectTo: `${import.meta.env.VITE_SITE_URL?.replace(/\/$/, '') || window.location.origin}/dashboard` },
      });
      if (error) throw error;
    } catch (error) {
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-500/20 outline-none pr-10"
                  placeholder="••••••••"
                  required
                />
                <button
